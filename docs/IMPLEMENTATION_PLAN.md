...
目标：理解知识图谱 RAG 与企业文档 RAG 平台如何组合底层能力。

LightRAG 任务：

1. 使用与自研链路相同的一批文档。
2. 观察实体与关系抽取结果。
3. 对比局部查询、全局查询和普通向量检索。
4. 重点测试跨文档关系、人物机构关系和全局总结问题。

RAGFlow 任务：

1. 导入相同的 PDF、Markdown 或 Word 文档。
2. 对比文档解析、分块与版面保留效果。
3. 配置关键词、向量、混合召回和重排。
4. 检查答案引用、页码和原文定位能力。
5. 分析其工作流中哪些模块对应本项目已有模块。

验收：对同一评估集输出自研链路、LightRAG 和 RAGFlow 的结果对照，说明各自更适合的问题类型及工程场景。
